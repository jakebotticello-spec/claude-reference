...
## 2026-06-01 — apparatus S25 (The Progenitor v1→v2 · retrieval engine conceptual design complete · seeding-council boot kit drafted · ANCHOR v18→v19)

**Scope:** Design + canon session. **No floor mutation. No invariant moved. No code committed.** New/updated canon authored (OC, for Jake to verify-commit-push): `active/apparatus/The_Progenitor_v2_2026-06-01.md` (NEW — supersedes v1, which stays on disk tombstoned-not-deleted), `active/apparatus/Seeding_Council_Boot_Kit_v0.1_2026-06-01.md` (NEW), ANCHOR v18→v19, this entry, the S25→S26 handoff.
...
- **S24-Claude cross-check on the boot kit (via Jake as bridge).** RATIFIED the bones; returned 6 non-structural changes queued for boot-kit v0.2: bless Q1 role-merge + add an independent-passes guard (recognize fences and count textures as separate passes over the same slice; never let a recognized fence color the texture read); upgrade Q2 (synthesis-gets-no-portrait) from fork to WALL (synthesis reconciles SHAPE never MEANING — the projection wall one scale up, the most dangerous place to put the portrait); add a canonical NO-card worked example (the most frequent + most fatigue-eroded decision, currently uncalibrated); add a content-neutral-slicing constraint as doctrine while deferring the mechanism (slicing by topic pre-categorizes the floor and becomes a projection surface — the thing the blind read exists to prevent); universalize the projection wall across ALL reading roles in §3 (otherwise a literal reading lets a fence-reader load the Lore Bible); give Jake a convergence-map audit appendix in the review model (so a systematically-over-carding reader isn't invisible when synthesis dedups its noise against real convergence).
...
**Why:** S24 authored the pointer-catalog law (The Progenitor v1) and laid the floor's predecessor work. S25 designed the engine OVER that catalog and folded the whole conceptual design back into the doctrine, then built the first applied artifact (the seeding-council boot kit) and ran it through the doctrine's own review pattern (Jake + an S24-Claude cross-check) before it exists. The session's defining texture: lore-first held end to end — every fork S25 posed that S24 had already settled was caught by Jake grabbing the prior spec rather than letting S25 re-derive it. The apparatus ran on its own prior author, which is exactly what the retrieval layer is being built to make automatic. Conceptual stack DONE; everything remaining is CC-build-against-the-live-floor.



**Scope:** Design + canon-hygiene session. **No floor mutation. No invariant moved.** New canon authored (OC, for Jake to verify-commit-push): `active/apparatus/The_Progenitor_v1_2026-06-01.md` (NEW), ANCHOR v17→v18, this entry, the S24→S25 handoff. One real git fix landed (loader branch merged to main — see below).

**Change(s):**
- **Git wound fixed — main made honest.** Discovered main carried `seed_shape_load.py` **v1.1 code under a v1.2 header**: a prior session had hand-edited the ANCHOR/loader's *claim* of the S23 append-only probe-fix, but the real `ctid`-targeted probe lived only on the unmerged branch `s23-loader-v1.2-fk-drop-probe-fix`. Caught by a read-only main-vs-branch diff on disk (the session-boot tarball read v16/v1.1 — CDN-stale; the live repo was already ahead). Fix: `git merge s23-loader-v1.2-fk-drop-probe-fix --no-ff` into main (reconciled loader + ANCHOR + CHANGELOG, no conflicts), verified the real `ctid` probe is on main (`WHERE ctid IN (SELECT ctid … LIMIT 1)`, not `WHERE false`), pushed `9759e55..67ad226`, deleted the branch local + remote (`-d` confirmed full-merge before deletion). Main is now honest: v1.2 code under a v1.2 header.
- **Phantom dependency KILLED.** The line "the three escalations (recall/kept/shared-memory) feed the retrieval layer, blocked on Jake's uploads" — copied **verbatim across 8 handoffs since S15→S16**, hardened into "the retrieval layer is blocked" — was traced on disk (`git grep` + history) to the SCDD merge-back doc: it names three **OPTIONAL repo-study tasks** (zippoxer/recall 186★, egroup-labs/kept 100★, a shared-memory cluster — *study other people's memory projects for borrowable mechanism*), never a prerequisite for designing the apparatus's own retrieval. Jake didn't recognize the terms because they were never something he owed. Retrieval is **UNBLOCKED.** ANCHOR Graveyard entry added so the blocker can't be re-copied forward.
- **The Progenitor authored (NEW canon file).** `active/apparatus/The_Progenitor_v1_2026-06-01.md` — the law of the pointer catalog (the retrieval layer's foundation). Two pointer kinds, one spine: **FENCE** (a decision/constraint; NOT a verdict but a *lineage* — an ordered chain of (span, date, call, why-as-live-predicate) links, latest current, earlier superseded-not-erased; Claude defends the *reasoning* not the *answer*, reads the chain to tell evolution from re-litigation) + **TEXTURE** (a pattern whose *volume is the signal*; representative span + count + spread; calibrates how-to-be-with-Jake). The shared spine — **append, keep lineage, supersede don't delete** — is the same architecture as the floor and the Claude-lineage, at three scales. Also: the bar ("default don't point"; complete over fences, sparse over motion; hundreds-to-low-thousands not tens-of-thousands), the two retrieval modes (in-session pointer-hit vs separate-window corpus-search; corpus-search = seeding at scale), the dumb-index/smart-reader line (selector MAY rank/cap, MAY NOT rewrite; RElitigation-prevention not PRElitigation-prediction), the reference-file workflow (Claude proposes / Jake verifies-commits-pushes; human-as-immutability-mechanism; tombstone-not-delete), and seeding guardrails (source is the FLOOR not chat_search; tree-aware spans; not-a-one-day-build). Canonical worked examples: the **1Pass wall** (fence-as-live-predicate: "still beta? re-check") and **Griffin's pickup** (frequency-as-fence + texture overlap — = picking Jake's kid up from school, 4× forgotten in 3 months post-meds, NOT a truck).
- **Named "The Progenitor" deliberately, not "Seed [anything].”** "Seed" is reserved in this project for the FLOOR's record-shape (`seed_shape_load.py`, `Seed_Shape_Test_Spec`); a "Seed Doctrine" would imply false kinship with the substrate layer. "Seeding" survives only for the act of populating the catalog with pointers.
- **ANCHOR v17→v18.** Banner + S24 READ-FIRST rewritten (floor LAID, probe scar recorded, four S24 actions); CURRENT STATE flipped BARE→LAID (325/24,138, append-only ENFORCED); loader lines to v1.2; new PROGENITOR block in INVARIANTS; NEXT MOVE #3 marked DONE, #4 rewritten (phantom→Progenitor + the engine); CONFIDENCE FLAGS updated (FK applied+proven on the laid floor, append-only landed-under-hollow-probe-then-revalidated, LOAD done); Graveyard entry for the phantom blocker; v18 + v17 enshrine footers appended (v17 folded in here since it post-dated the boot tarball).
- **Floor re-confirmed live** — `seed_shape_load.py --dry-run` reports tables EXIST (the correct laid-floor signal); never `--execute`.

**Why:** S23 closed the substrate arc (floor laid). S24 is the first real step into the retrieval layer — its foundational law — plus the canon hygiene that step exposed (a stale git state and an 8-handoff-old phantom dependency). Lore-first held all session: the Why/Doctrine/build were front-loaded precisely because the retrieval layer's deepest job is keeping a fresh-boot Claude from re-litigating settled ground — run by hand, that's exactly what front-loading the lore does.
